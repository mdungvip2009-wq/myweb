...
from api_client import (
    ...
    day_history, day_history_to_time, pending_orders, BASE_URL, _headers  # <-- THÊM day_history_to_time VÀO ĐÂY
)
...
def _check_order_result(token: str, order_id: str, type_mode: str) -> Dict[str, Any] | None:
    ...
    try:
        # Tính toán thời gian: 24h trước đến hiện tại
        import time
        current_time = int(time.time())
        start_time = current_time - (24 * 60 * 60)  # 24h trước
        
        # Gọi API lịch sử mới với khoảng thời gian
        r = requests.post(
            f"{BASE_URL}/api/binaryOption/dayHistoryOrderToTime",
            json={
                "limit": 50, 
                "page": 1, 
                "type": type_mode,
                "timeStart": str(start_time),
                "timeEnd": str(current_time)
            },
            headers=_headers(token),
            timeout=10
        )
            ...
            if js.get("status") and js.get("data", {}).get("array"):
                for order in js["data"]["array"]:
                    if str(order.get("idChart")) == str(order_id):
                        # Theo thông tin của bạn: resultProfit = 0 là thua, > 0 là win
                        result_profit = float(order.get("resultProfit", 0))
                        draw = int(order.get("draw", 0))
                        
                        if draw == 1:
                            return {"result": "draw", "pnl": 0}
                        elif result_profit > 0:
                            return {"result": "win", "pnl": result_profit}
                        else:
                            # Thua: tính toán lỗ dựa trên amount
                            amount = float(order.get("amount", 0))
                            return {"result": "lose", "pnl": -amount}
    except Exception:
        pass
    return None
...
def api_admin_update_leaderboard():
    """Update a user's stats in the leaderboard"""
    try:
        data = request.get_json()
        username = data.get('username', '').strip()
        new_name = data.get('newName', '').strip() or username
        ...
        if win_rate < 0 or win_rate > 100:
            return jsonify({"ok": False, "error": "Win rate must be between 0 and 100"}), 400

        # Load current leaderboard
        current_data = _load_server_leaderboard()
        leaderboard = current_data.get('data', [])
        ...
        for entry in leaderboard:
            if entry['name'].lower() == username.lower():
                entry['name'] = new_name
                entry['winRate'] = win_rate
                entry['profit'] = profit
                entry['score'] = f"{win_rate:.2f}"
                entry['profitText'] = f"+${profit}"
                user_found = True
                break

        if not user_found:
            return jsonify({"ok": False, "error": "User not found in leaderboard"}), 404
        ...
        # Keep only top 10
        current_data['data'] = leaderboard[:10]

        # Save changes
        with open(LEADERBOARD_FILE, 'w', encoding='utf-8') as f:
            json.dump(current_data, f, ensure_ascii=False, indent=2)

        return jsonify({"ok": True, "message": "Leaderboard updated successfully"})
...
def api_bot_start():
    try:
        ...
        fixed_amount = float(data.get("fixedAmount", 0))
        take_profit = float(data.get("takeProfit", 0))
        stop_loss = float(data.get("stopLoss", 0))

        if not strategy or investment <= 0 or fixed_amount <= 0:
            return jsonify({"ok": False, "error": "Thông tin không hợp lệ"}), 400

        if fixed_amount > investment:
            return jsonify({"ok": False, "error": "Số tiền mỗi lệnh không được lớn hơn vốn đầu tư"}), 400

        # TODO: Implement bot trading logic here
        # For now just return success
        return jsonify({
            "ok": True,
            "message": f"Đã kích hoạt bot {strategy}",
            "data": {
                "strategy": strategy,
...
def page_vip():
    ...
    return render_template("vip.html", token=token)
...
def api_history():
    try:
        ...
        # Sử dụng API mới với khoảng thời gian
        js = day_history_to_time(token, tmode, str(start_time), str(current_time), limit=limit, page=1)
        arr = js["data"]["array"]

        def pnl(r):
            # Theo thông tin của bạn: resultProfit = 0 là thua, > 0 là win
            result_profit = float(r.get("resultProfit", 0))
        ...
        rows = [{
            ...
            "resultProfit": float(r.get("resultProfit", 0)),
            "pnl": pnl(r),
        } for r in arr]
...
def api_day_history_order():
    ...
    try:
        ...
        # Kiểm tra nếu có timeStart và timeEnd thì dùng API mới
        time_start = data.get("timeStart")
        time_end = data.get("timeEnd")
        
        if time_start and time_end:
            # Sử dụng API mới với khoảng thời gian
            js = day_history_to_time(token, type_mode, time_start, time_end, limit=limit, page=page)
        else:
            # Sử dụng API cũ
...
# ---- API lịch sử giao dịch theo thời gian (mới) ----
@app.route("/api/dayHistoryOrderToTime", methods=["POST"])
@limit_concurrent
def api_day_history_order_to_time():
    """API lịch sử giao dịch theo khoảng thời gian"""
    try:
        token = pick_token()
        if not token:
            return jsonify({
