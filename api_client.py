...
def day_history_to_time(token: str, type_mode: str, time_start: str, time_end: str, limit: int = 10, page: int = 1):
    ...
    r = requests.post(f"{BASE_URL}/api/binaryOption/dayHistoryOrderToTime",
                      json=payload, headers=_headers(token), timeout=10)
    r.raise_for_status()
    return r.json()
